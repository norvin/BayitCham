...
<article id="node-<?php print $node->nid; ?>" class="<?php print $classes; ?> clearfix"<?php print $attributes; ?>>
  ...
  <?php //dsm($content); ?>
  <?php if(!$teaser){?>
  <!--hide the title of  the gallery-->
  <style>h1#page-title{display:none;}</style>
  <div class="innertomaingallery">
    <?php print l('<div class="tg-content">' . t('TO THE MAIN GALLERY >') . '</div>','galleries',array('html'=>true))?>
  </div>
  
  <?php
  // get the node id of the next gallery..
  module_load_include('inc', 'media_gallery', 'media_gallery.pages');
  $v = taxonomy_term_load(variable_get('media_gallery_default_collection_tid', 0));
  $d = media_gallery_list_galleries($v);
  $nodes = $d['nodes'];
  ...
  if(!(is_numeric($nextnid))){ // if the nextnid is not a number set it to the first value of the array of nids
    $nextnid = $nodes[0];
  }
  ?>
  
  <div class="gallerytitle">
    <div class="gal-title-date">
      <?php if ($title): ?>
        <div<?php print $title_attributes; ?>>
          <a href="<?php print $node_url; ?>" rel="bookmark"><?php print $title; ?></a>
        </div>
        <div class="gal-date"><?php print format_date($created,'custom','D, F d, Y'); ?></div>
      <?php endif; ?>
    </div>
    <div class="gal-next-gal"><?php if($nextnid!=0) print l(t('Next Gallery >'),'node/'.$nextnid); ?></div>
  </div>
  <?php } ?>

  <div class="field field-name-media-gallery-media">
